    ...
    def _background_track_results(self, original_cv: str, job_description: str,
        ...
        try:
            ...
            self._log_generation_to_sheets({
                ...
                'job_keywords': job_keywords[:200],  # Limit length
    ...
    def _parse_cv_content(self, cv_content: str) -> dict:
        ...
        for line in lines[:5]:  # Check first 5 lines
            ...
            if line and len(line.split()) <= 4 and not any(char in line for char in '@+.'):
                # Likely a name if it's short and no email/phone symbols
        ...
        phone_patterns = [
            ...
            r'\b\d{2,4}[-.\s]?\d{3}[-.\s]?\d{3,4}\b'  # International
        ]
        phone = ""
        ...
        for i, line in enumerate(cv_lines):
            if any(indicator in line.lower() for indicator in skill_indicators):
                # Found skills section, get next few lines
                skills_lines = []
        ...
        # Extract education (look for degree, university)
        education = ""
        ...
        return {
            'name': potential_name[:50],  # Limit length
            'email': email,
            'phone': phone[:20],
        ...
        try:
            ...
            try:
                ...
                if not headers or len(headers) < 10:
                    # Create detailed headers
                    new_headers = [
                        'Timestamp', 'Session_ID', 'Action', 'Filename', 'Full_CV_Text',
                        'Name', 'Email', 'Phone', 'Location', 'Skills', 'Experience_Years',
                        'Education', 'CV_Word_Count', 'Company_Name', 'Generated_CV', 
                        'Cover_Letter', 'Job_Keywords', 'Status'
                    ]
                    worksheet.clear()
            ...
            row = [
                ...
                data['action'],
                data['filename'],
                data['full_cv_text'][:5000],  # Limit to 5000 chars for Google Sheets
                data['name'],
                data['email'],
                data['phone'],
                data['location'],
    ...
    def _log_generation_to_sheets(self, data: dict):
        ...
        try:
            ...
            for i, row in enumerate(all_values):
                if len(row) > 1 and row[1] == session_id:  # Session_ID is in column B (index 1)
                    # Update the row with generation data
                    row_num = i + 1
